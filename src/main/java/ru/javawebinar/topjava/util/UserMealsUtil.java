...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : meals) {
            ...
            sumCaloriesPerDay.put(mealDate, sumCaloriesPerDay.getOrDefault(mealDate, 0) + meal.getCalories());
        }
        List<UserMealWithExcess> mealExcesses = new ArrayList<>();
        for (UserMeal meal : meals) {
            LocalDateTime localDateTime = meal.getDateTime();
            if (TimeUtil.isBetweenHalfOpen(localDateTime.toLocalTime(), startTime, endTime)) {
                mealExcesses.add(new UserMealWithExcess(localDateTime, meal.getDescription(), meal.getCalories(),
                        sumCaloriesPerDay.get(localDateTime.toLocalDate()) > caloriesPerDay));
            }
        }
        return mealExcesses;
    }
